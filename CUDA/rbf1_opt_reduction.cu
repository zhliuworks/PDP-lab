/*
 *  RBF kernel computation -- optimize reduction (1), based on baseline (0)
 *
...
__global__ void reduction_0(int *matIn, int *matOut, int matIn_size) {
...
__global__ void reduction_1(int *matIn, int *matOut, int matIn_size) {
    /* optimized_1: strided index and non-divergent branch */
...
__global__ void reduction_2(int *matIn, int *matOut, int matIn_size) {
    /* optimized_2: sequential addressing */
    extern __shared__ int matS[];
...
__global__ void reduction_3(int *matIn, int *matOut, int matIn_size) {
    /* optimized_3: first add during load */
    extern __shared__ int matS[];

    // perform first level of reduction
    // reading from global memory, writing to shared memory
    // note: halve the number of blocks!
    int tx = threadIdx.x;
...
__global__ void reduction_4(int *matIn, int *matOut, int matIn_size) {
    /* optimized_4: unroll the last warp */
    extern __shared__ int matS[];
...
__host__ int RBF_Compute_GPU_0(int *matA, int *matB, int size) {
    ...
    for (int n = 0; n < REPEATED_TIMES; n++) {
        ...
        reduction_0<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, size);
        cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
        ...
        while (curr_size > 1) {
            ...
            // reduction again
            reduction_0<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, curr_size);
            cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
...
__host__ int RBF_Compute_GPU_1(int *matA, int *matB, int size) {
    ...
    for (int n = 0; n < REPEATED_TIMES; n++) {
        ...
        // reduction_1
        reduction_1<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, size);
        cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
        ...
        while (curr_size > 1) {
            ...
            dim3 dimGrid(curr_size / NUM_THREADS + 1, 1, 1);
            // reduction_1
            reduction_1<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, curr_size);
            cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
            curr_size /= NUM_THREADS;
        }
        CUDA_CHECK(cudaMemcpy(&res, matOd, sizeof(int), cudaMemcpyDeviceToHost));
...
__host__ int RBF_Compute_GPU_2(int *matA, int *matB, int size) {
    ...
    for (int n = 0; n < REPEATED_TIMES; n++) {
        ...
        // reduction_2
        reduction_2<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, size);
        cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
        ...
        while (curr_size > 1) {
            ...
            dim3 dimGrid(curr_size / NUM_THREADS + 1, 1, 1);
            // reduction_2
            reduction_2<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, curr_size);
            cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
            curr_size /= NUM_THREADS;
        }
        CUDA_CHECK(cudaMemcpy(&res, matOd, sizeof(int), cudaMemcpyDeviceToHost));
...
__host__ int RBF_Compute_GPU_3(int *matA, int *matB, int size) {
    ...
    for (int n = 0; n < REPEATED_TIMES; n++) {
        ...
        // reduction_3, halve the number of blocks
        dim3 dimGrid(size / NUM_THREADS / 2, 1, 1);
        reduction_3<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, size);
        cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());

        int curr_size = size / NUM_THREADS / 2;
        while (curr_size > 1) {
            CUDA_CHECK(cudaMemcpy(matId, matOd, sizeof(int) * curr_size, cudaMemcpyDeviceToDevice));
            dim3 dimGrid(curr_size / NUM_THREADS + 1, 1, 1);
            // reduction_3
            reduction_3<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, curr_size);
            cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
            curr_size /= NUM_THREADS;
        }
        CUDA_CHECK(cudaMemcpy(&res, matOd, sizeof(int), cudaMemcpyDeviceToHost));
...
__host__ int RBF_Compute_GPU_4(int *matA, int *matB, int size) {
    ...
    for (int n = 0; n < REPEATED_TIMES; n++) {
        ...
        // reduction_4, halve the number of blocks
        dim3 dimGrid(size / NUM_THREADS / 2, 1, 1);
        reduction_4<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, size);
        cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
        ...
        while (curr_size > 1) {
            ...
            dim3 dimGrid(curr_size / NUM_THREADS + 1, 1, 1);
            // reduction_4
            reduction_4<<<dimGrid, dimBlock, SHARED_MEM_SIZE>>>(matId, matOd, curr_size);
            cudaDeviceSynchronize(); CUDA_CHECK(cudaGetLastError());
            curr_size /= NUM_THREADS;
        }
        CUDA_CHECK(cudaMemcpy(&res, matOd, sizeof(int), cudaMemcpyDeviceToHost));
